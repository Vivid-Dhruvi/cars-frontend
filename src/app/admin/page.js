...
export default function AdminDashboardPage() {
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] text-slate-900 font-sans flex flex-col lg:flex-row antialiased">
      ...
      <aside className="w-full lg:w-64 bg-white border-r border-slate-200 p-5 flex flex-col justify-between shrink-0 lg:min-h-screen">
        <div className="flex flex-col gap-6">
          ...
          <div className="flex items-center gap-2.5 pb-4 border-b border-slate-100">
            ...
            <button ref={menuRef} type="button" aria-expanded={navigationOpen} aria-controls="admin-navigation" onClick={() => setNavigationOpen(open => !open)} className="ml-auto min-h-11 rounded-xl border border-slate-200 px-4 text-sm font-bold lg:hidden">{navigationOpen ? 'Close menu' : 'Menu'}</button>
          </div>

          {/* LISTING CONTENT GROUP */}
          <div id="admin-navigation" className={`${navigationOpen ? 'flex' : 'hidden'} flex-col gap-4 lg:flex`}>
            <span className="text-xs font-bold text-slate-400 uppercase tracking-wider px-2">Listing Content</span>
            ...
            <span className="text-xs font-bold text-slate-400 uppercase tracking-wider px-2 mt-3">Settings & System</span>
            <nav className="flex flex-col gap-1">
              <button
        ...
        <div className="pt-4 border-t border-slate-100 hidden lg:flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <span className="text-xs text-slate-400">admin@rentscan.ae</span>
            </div>
          </div>
      ...
      <main className="min-w-0 flex-1 p-4 sm:p-6 xl:p-8 flex flex-col gap-6">
          ...
          <div className="flex min-w-0 flex-wrap items-center gap-3 xl:flex-1 xl:justify-end">
            <div className="relative min-w-0 flex-1 basis-full sm:basis-auto xl:max-w-64">
              ...
              <input 
                ...
                aria-label="Search inspections" className="min-h-11 pl-9 pr-4 bg-slate-50 border border-slate-200 rounded-xl text-xs focus:outline-none focus:ring-2 focus:ring-[#0F172A] w-full"
              />
            </div>
            ...
            <Link href="/" className="min-h-11 shrink-0 px-4 bg-[#0F172A] hover:bg-[#0F172A] text-white rounded-xl text-xs font-bold flex items-center gap-2 shadow-xs transition-colors">
        ...
        {adminTab === 'overview' && (
          <div className="flex flex-col gap-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              ...
              <button className="min-h-11 px-3.5 bg-white border border-slate-200 rounded-xl text-xs font-semibold text-slate-700 flex items-center gap-2 hover:bg-slate-50 shadow-xs">
              ...
              <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs flex flex-col justify-between">
                <div>
                  <div className="flex flex-wrap items-center justify-between gap-2 text-xs text-slate-500 mb-1">
                    ...
                    <span className="text-xs font-bold text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-md border border-emerald-200">↗ +57.1%</span>
                  </div>
                  <span className="text-3xl font-extrabold text-slate-900">3,890</span>
                </div>
                <div className="mt-4 pt-3 border-t border-slate-100 flex flex-col items-start gap-2 text-xs text-slate-400">
              ...
              <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs flex flex-col justify-between">
                <div>
                  <div className="flex flex-wrap items-center justify-between gap-2 text-xs text-slate-500 mb-1">
                    ...
                    <span className="text-xs font-bold text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-md border border-emerald-200">↗ +83.4%</span>
                  </div>
              ...
              <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs flex flex-col justify-between">
                <div>
                  <div className="flex flex-wrap items-center justify-between gap-2 text-xs text-slate-500 mb-1">
                    ...
                    <span className="text-xs font-bold text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-md border border-emerald-200">↗ +71.4%</span>
                  </div>
              ...
              <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs flex flex-col justify-between">
                <div>
                  <div className="flex flex-wrap items-center justify-between gap-2 text-xs text-slate-500 mb-1">
                    ...
                    <span className="text-xs font-bold text-[#0F172A] bg-slate-100 px-2 py-0.5 rounded-md border border-slate-200">68.4%</span>
                  </div>
            ...
            <div className="bg-white rounded-3xl p-4 sm:p-6 border border-slate-200 shadow-xs flex flex-col gap-4">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div>
                ...
                <div className="flex flex-wrap items-center gap-1 bg-slate-100 p-1 rounded-xl border border-slate-200 text-xs font-semibold">
                  <button className="min-h-11 px-3 py-2 bg-white text-slate-900 rounded-lg shadow-xs">Last 3 months</button>
                  <button className="min-h-11 px-3 py-2 text-slate-500 hover:text-slate-900">Last 30 days</button>
                  <button className="min-h-11 px-3 py-2 text-slate-500 hover:text-slate-900">Last 7 days</button>
                </div>
              </div>
            ...
            <div className="bg-white rounded-3xl p-4 sm:p-6 border border-slate-200 shadow-xs flex flex-col gap-4">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                ...
                <button className="min-h-11 px-3.5 bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold text-xs rounded-xl border border-slate-200">
                  Refresh Queue
                </button>
              </div>
              ...
              <div role="region" aria-label="Recent inspections table" tabIndex={0} className="max-w-full overflow-x-auto">
                <table className="w-full text-left text-xs">
                  <thead>
                    <tr className="border-b border-slate-200 text-slate-400 uppercase tracking-wider text-xs bg-slate-50">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {[
                      ...
                      <tr key={idx} className="hover:bg-slate-50">
                        ...
                        <td className="p-3">
                          <span className={`px-2.5 py-1 rounded-full text-xs font-bold ${row.status === 'PAID' ? 'bg-emerald-100 text-emerald-800' : 'bg-slate-100 text-slate-600'}`}>
        ...
        {/* TAB 5: SETTINGS */}
        {adminTab === 'settings' && (
          <div className="bg-white rounded-3xl p-4 sm:p-6 border border-slate-200 shadow-xs flex flex-col gap-4 max-w-xl">
            <h3 className="font-bold text-base text-slate-900">System Parameters</h3>
            <div className="flex flex-col gap-3 text-xs">
              <div>
                <label htmlFor="report-price" className="font-bold text-slate-700 block mb-1">Full Report Paywall Price ($USD)</label>
                <input id="report-price" type="text" defaultValue="3.99" className="w-full min-h-11 px-3 border border-slate-200 rounded-xl bg-slate-50 font-semibold" />
              </div>
              <button onClick={() => alert("Settings saved!")} className="w-fit min-h-11 px-5 bg-slate-900 text-white font-bold rounded-xl mt-2">
                Save Settings
              </button>
            </div>
          </div>
